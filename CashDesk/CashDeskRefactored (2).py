...
def create_gate_buttons(frame):
    global gatesList
    counter = 1
    for x in gatesList:
        name=tkinter.Button(frame, text=f"Bramka {counter}" , command=lambda: unblock_gate(x, main_frame))
        name.pack()
        counter+=1
...
def connect_to_broker():
    ...
    client.username_pw_set(username='cashdesk', password='cashdesk')
